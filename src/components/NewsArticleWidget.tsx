...
const NewsArticleWidget = () => {
  ...
  return (
    <div className="glass-strong rounded-3xl liquid-border shadow-xl shadow-black/50 flex flex-row items-stretch w-full h-24 min-h-[96px] max-h-[96px] p-0 overflow-hidden hover:border-white/20 smooth-transition">
      {loading ? (
        <div className="animate-pulse flex flex-row items-stretch w-full h-full">
          <div className="w-24 min-w-24 max-w-24 h-full glass" />
          <div className="flex flex-col justify-center flex-1 px-3 py-2 gap-2">
            <div className="h-4 w-3/4 rounded glass" />
            <div className="h-3 w-1/2 rounded glass" />
          </div>
        </div>
      ) : error ? (
        ...
        >
          ...
          <div className="flex flex-col justify-center flex-1 px-3 py-2">
            <div className="font-semibold text-xs text-white leading-tight line-clamp-2 mb-1">
              {article.title}
            </div>
